...
public class BonusFeast extends Structure {
	...
	public BonusFeast(Manager manager, int size) {
		super(manager);

		this.chestsData = new ArrayList<>();
		this.blockData = new ArrayList<>();

		location = Bukkit.getWorlds().get(0).getHighestBlockAt(getCoord(300), getCoord(150)).getLocation();
		if (location.getY() >= 90) {
			location.setY(72);
		}
	}
	...
	public void spawnFeast() {
		...
		new BukkitRunnable() {
			public void run() {
				if (!isSpawned) {
					isSpawned = true;
					Bukkit.broadcastMessage("§C§lBONUS-FEAST §fO feast-bônus foi spawnado em algum lugar!");
					spawnChests();
					cancel();
				}
			}
		...
		for (int x = -20; x <= 20; x++) {
			for (int z = -20; z <= 20; z++) {
				...
				if (feast.distance(location) <= 20) {
					...
					do {
						getManager().getCoreManager().getBO2().setBlockFast(loc, Material.AIR, (byte) 0);
						loc.setY(loc.getY() + 1);
						remover = loc.getBlock();
					} while (loc.getY() < loc.getWorld().getMaxHeight());

					getManager().getCoreManager().getBO2().setBlockFast(feast, Material.GRASS, (byte) 0);
