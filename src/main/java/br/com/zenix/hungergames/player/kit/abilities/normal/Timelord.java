...
public class Timelord extends Kit {
	...
	public void playEffectSquare(Player player, Location loction, boolean bool) {
		for (int i = 0; i <= 50; i += ((!bool && (i == 50)) ? 2 : 1)) {
			ParticleType.ANGRY_VILLAGER.setParticle(player, loction, i, i, i, 0, 10);
		}
	}
		...
		if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
			if (hasKit(player)) {
				if (isKitItem(player.getItemInHand(), "§eTimelord")) {
					...
					playEffectSquare(player, player.getLocation(), false);
