...
public class AwardManager extends Management {
	...
	public boolean generate(Account account) {
		createInventory(account.getPlayer());
		return true;
	}
	...
	private void givePrize(Player player, Surprise crate) {
		...
		attachment.setPermission("hgkit." + reward.getReward().toLowerCase(), true);

		suject.add(gamer.getPlayer());
		gamer.setSurpriseKit(getManager().getKitManager().getKit(reward.getReward().toLowerCase()));

		account.sendMessage("§6§lKIT SURPRESA §fVocê ganhou o kit " + reward.getReward().toUpperCase() + "!");
	}
